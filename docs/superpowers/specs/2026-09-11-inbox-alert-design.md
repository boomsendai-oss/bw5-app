...
| Pushoverへの送信失敗 | 他のアカウントの鍵（BOOM優先）で件名に〔アカウント名〕を付けて送り直す。全部だめなら `notified_at` を空のまま残し、次回に再送。自分の鍵で送れなかったことは `push_failed_at` に残し、朝のまとめの稼働欄に「要確認（通知の送信に失敗・続く場合はPushoverの鍵を確認）」と出す（朝のまとめをそのアカウントの鍵で送れたら消す）。一度失敗した鍵はその回は全アカウントで使わない。朝のまとめ自体も他の鍵で送り直す |
