...
| 未対応の管理 | 返信する、Gmailでアーカイブする、またはゴミ箱・迷惑メールに入れると消える（受信トレイを通らなかったメールは、通知が届いていれば読んだら消える）。それまで毎朝のまとめに残る |
...
- 優先度: 通常（0）。おやすみモード中は鳴らさない（TARO確認済み）。おやすみモードを突破する設定（1）・確認するまで鳴らし続ける設定（2）は使わない
...
- ゴミ箱・迷惑メール: そのメールに TRASH か SPAM ラベルがある
- 受信時に受信トレイに無かったメール（振り分けで受信トレイを通らない等）: UNREAD ラベルが無くなったら（読んだら）閉じる
- 通知がまだ届いていない行（Pushover の送信失敗）は、返信・ゴミ箱・迷惑メールでしか閉じない（既読やアーカイブで閉じると再送されなくなるため）
...
| Pushoverへの送信失敗 | 他のアカウントの鍵（BOOM優先）で件名に〔アカウント名〕を付けて送り直す。全部だめなら `notified_at` を空のまま残し、次回に再送。自分の鍵で送れなかったことは `push_failed_at` に残し、朝のまとめの稼働欄に「要確認（通知の送信に失敗・続く場合はPushoverの鍵を確認）」と出す（朝のまとめをそのアカウントの鍵で送れたら消す）。鍵そのものの失敗（時間切れ・通信エラー・鍵の無効など）が一度起きた鍵は、その回は全アカウントで使わない（メール1通だけの失敗では外さない）。朝のまとめ自体も他の鍵で送り直す |
| 仕組みごと止まる（Worker/Vercel） | 朝のまとめが届かない＝止まっている合図（TAROに周知） |

## 9.5 決め打ちルール（2026-09-14 ドライラン結果を受けて追加）

過去30日を本番ドライランで判定し、TAROが全件確認した結果、**同じ差出人・同じ件名でもAIの判定がぶれる**ことが分かった（体験予約の通知12通が now 10 / digest 1 / count 1 に割れ、12回に1回は消えていた）。そこで `src/lib/inboxAlert/rules.ts` にAIを通さない決め打ちの層を置く。

- **必ず鳴らす**: 体験レッスン/見学の予約（Lステップ）、予約のキャンセル・日程変更、会員のプラン契約・変更（hacomono）、口座振替不能、乗っ取りの兆候（端末・パスキー・アクセストークンの追加、新しい環境からのログイン）
- **朝のまとめに固定**: 口座振替・決済の処理、請求書・課金、行政手続き（期限つきは鳴らす）、ログイン通知・確認コード、Search Console、障害情報、Amazonの出品者とのやりとり
- **AIに読ませない**: 銀行・証券8ドメインの通常の取引通知、SNS・販促メルマガ、指定アドレスからの営業メール。ただし件名に「不成立／不能／出金停止／実施されませんでした／ご利用不可／未精算（解除を除く）／Action required」があれば鳴らす
- **必ず本文まで読む**: 営業と本物の連絡が同じ差出人で来るもの（リベシティ、form.run、hacomonoの会社ドメイン）

この結果、AIが読む通数は1,513→616通（約6割減・月2,670円→1,200円前後）、鳴る通知は1日2〜3回に収まる見込み。
...
- 事前検証（通知は送らない）: 過去30日の実メールでローカルに dry-run し、「すぐ鳴らす」になったメールの件数と一覧を画面上でTAROと確認する（件名は保存しない）。目的は判定基準の最初のラインを決めること。AI費用は1回で約950〜1,500円（ほぼ1ヶ月分）で、**TARO了承済み（2026-09-11）**
