...
export default function StreamClient() {
  ...
  // 販売していないときは案内を出さず、黙ってイベントページへ戻す(TARO 2026-09-17
  // 「しれっと消して、誰も見れないようにしてしまえばいい」)。通常はサーバ側の
  // page.tsx で先に弾いているので、ここへは来ない。設定が途中で変わったときの保険。
  if (!ctx || !ctx.open) {
    if (typeof window !== 'undefined') window.location.replace('/bf6');
