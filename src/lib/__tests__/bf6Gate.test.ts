...
describe('名前順(あいうえお順)', () => {
  ...
  // 出場者の「本名(カタカナ)」は親子で名字が同じなので、それを読みとして使う(TARO 2026-09-11「名前でソート」)。
  // ⚠️ 「ダンサーネームのフリガナ」(MCが呼ぶ読み・ヒマリ等)を使うと名字の順にならない(本番で並びが崩れた)。
  it('本名カタカナをひらがなにした読みを並びの鍵にする', () => {
    expect(gateSortKey({ buyerName: '渡辺 母', realNameKana: ['ワタナベ ユイ'] })).toBe('わたなべゆい');
  });
  it('本名カタカナが無ければ申込者の名前を使う(観覧だけの申込)', () => {
    expect(gateSortKey({ buyerName: '山田 太郎', realNameKana: [] })).toBe('山田太郎');
  });
  it('読みの順に並ぶ', () => {
