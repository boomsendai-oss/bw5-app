...
import { isAlwaysFullSender, isDropSender } from './rules';
...
export function decideReadMode(meta: MessageMeta): ReadMode {
  ...
  // 決め打ちルール(rules.ts)を先に見る(鳴らす分は run.ts が先に拾う)。
  // 止める指定は「全文を読む」より先に見る(同じドメインでも、宣伝のアドレスだけ止められるように)
  if (isDropSender(address, h['subject'] ?? '')) return 'count_only';
  if (isAlwaysFullSender(address)) return 'ai_full';
